...
#define PROJ_W_CLIP     (2.0f * PROJ_Z_FAR * PROJ_Z_NEAR / (PROJ_Z_NEAR - PROJ_Z_FAR))
    ...
    EMSCRIPTEN_RESULT webContextMakeCurrentResult = EMSCRIPTEN_RESULT_FAILED;
#endif

int32 gWidth, gHeight;

// Desktop targets load OpenGL entry points dynamically. WebGL2 exposes the
// same ES3 functions directly, so declaring same-named function pointers there
// would shadow/redefine the browser API.
...
PFNGLDELETEVERTEXARRAYPROC          glDeleteVertexArrays;
...
void renderInit()
{
    ...
    attrs.majorVersion = 2; // WebGL 2 / OpenGL ES 3 surface
    attrs.minorVersion = 0;
    attrs.explicitSwapControl = false;
    webContext = emscripten_webgl_create_context("#canvas", &attrs);
    if (webContext <= 0)
        return;
    webContextMakeCurrentResult = emscripten_webgl_make_context_current(webContext);
    if (webContextMakeCurrentResult != EMSCRIPTEN_RESULT_SUCCESS)
    {
...
#elif defined(__EMSCRIPTEN__)
    // The browser presents WebGL frames at the event-loop boundary. Explicit
    // swap is reserved for offscreen/threaded contexts and is intentionally
    // not enabled in this single-threaded probe.
#endif
}
