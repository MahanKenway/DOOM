...
The adapter is built by the RIFTWAD GitHub Pages workflow into `dist/openresident/`. Its public host is `openresident.html`, which only validates WebAssembly and WebGL2 renderer setup. It does not initialise a game session and it does not contain, obtain, request, mount or distribute any Resident Evil game data.

The included upstream `LICENSE` applies to the copied OpenResident source material. RIFTWAD-specific adapter changes are also supplied under the same BSD 2-Clause terms.
